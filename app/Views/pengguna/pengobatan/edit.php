    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbarsFurni">
            <ul class="custom-navbar-nav navbar-nav ms-auto mb-2 mb-md-0">
                <li class="nav-item active"><a class="nav-link" href="<?= base_url('pengguna/dashboard') ?>">Beranda</a></li>
...
<div class="container mt-5">
  <div class="row">
    <div class="col-md-8 offset-md-2">
      ...
      <form method="POST" action="<?= base_url('pengguna/pengobatan/update/' . $pengobatan['id_pengobatan']); ?>">
        <?= csrf_field(); ?>
        ...
        <div class="form-group">
          ...
          <input 
              type="text" 
              class="form-control <?= isset(session()->getFlashdata('errors')['nama_obat']) ? 'is-invalid' : ''; ?>" 
              id="nama_obat" 
              name="nama_obat" 
              value="<?= old('nama_obat', $pengobatan['nama_obat']); ?>" 
              required>
          <?php if (isset(session()->getFlashdata('errors')['nama_obat'])): ?>
            <div class="invalid-feedback"><?= session()->getFlashdata('errors')['nama_obat']; ?></div>
          <?php endif; ?>
        </div>
        ...
        <div class="form-group">
            ...
            <input 
                type="datetime-local" 
                class="form-control <?= isset(session()->getFlashdata('errors')['waktu_pengobatan']) ? 'is-invalid' : ''; ?>" 
                id="waktu_pengobatan" 
                name="waktu_pengobatan" 
                value="<?= old('waktu_pengobatan') ? old('waktu_pengobatan') : date('Y-m-d\TH:i', strtotime($pengobatan['waktu_pengobatan'])); ?>" 
                required>
            <?php if (isset(session()->getFlashdata('errors')['waktu_pengobatan'])): ?>
                <div class="invalid-feedback"><?= session()->getFlashdata('errors')['waktu_pengobatan']; ?></div>
            <?php endif; ?>
        </div>

        <div class="form-group">
          <label for="durasi_pengobatan">Durasi Pengobatan</label>
          <input 
              type="number" 
              step="1" 
              ...
              value="<?= old('durasi_pengobatan', $pengobatan['durasi_pengobatan']); ?>" 
              required>
          <?php if (isset(session()->getFlashdata('errors')['durasi_pengobatan'])): ?>
            <div class="invalid-feedback"><?= session()->getFlashdata('errors')['durasi_pengobatan']; ?></div>
          <?php endif; ?>
        </div>
        ...
        <div class="form-group">
          ...
          <input 
              type="text" 
              class="form-control <?= isset(session()->getFlashdata('errors')['dosis']) ? 'is-invalid' : ''; ?>" 
              id="dosis" 
              name="dosis" 
              value="<?= old('dosis', $pengobatan['dosis']); ?>" 
              required>
          <?php if (isset(session()->getFlashdata('errors')['dosis'])): ?>
            <div class="invalid-feedback"><?= session()->getFlashdata('errors')['dosis']; ?></div>
          <?php endif; ?>
        </div>
        ...
        <div class="form-group">
            ...
            <select 
                class="form-control <?= isset(session()->getFlashdata('errors')['id_pemeriksaan']) ? 'is-invalid' : ''; ?>" 
                id="id_pemeriksaan" 
                name="id_pemeriksaan" 
                required>
                ...
                <option 
                    value="<?= $item['id_pemeriksaan']; ?>" 
                    <?= old('id_pemeriksaan', $pengobatan['id_pemeriksaan']) == $item['id_pemeriksaan'] ? 'selected' : ''; ?>>
                    <?= $item['id_pemeriksaan'] . ' - ' . $item['diagnosa']; ?>
                </option>
                <?php endforeach; ?>
            </select>
            <?php if (isset(session()->getFlashdata('errors')['id_pemeriksaan'])): ?>
                <div class="invalid-feedback"><?= session()->getFlashdata('errors')['id_pemeriksaan']; ?></div>
            <?php endif; ?>
        </div>
