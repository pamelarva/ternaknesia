...
<head>
  ...
  <title>Ternaknesia - Pengobatan</title>
</head>
    ...
    <div class="container">
      ...
      <div class="row mb-3">
        ...
        <div class="col-12 text-right" style="position: relative;">
          <a href="<?= base_url('pengguna/pengobatan/create'); ?>" 
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php if (!empty($pengobatan)) : ?>
                            <?php $no = 1; foreach ($pengobatan as $item) : ?>
                                <tr>
                                    <td><?= $no++ ?></td>
                                    <td><?= esc($item['nama_obat']) ?></td> <!-- Nama Obat -->
                                    <td><?= date('d-m-Y H:i', strtotime($item['waktu_pengobatan'])) ?></td> <!-- Waktu Pengobatan -->
                                    <td><?= esc($item['dosis']) ?> ml</td> <!-- Dosis -->
                                    <td><?= esc($item['durasi_pengobatan']) ?> hari</td> <!-- Id Pemeriksaan -->
                                    <td>
                                        <!-- Edit Button -->
                                        <a href="<?= base_url('pengguna/pengobatan/edit/' . $item['id_pengobatan']) ?>" class="btn btn-primary btn-sm" title="Edit">
                                            <i class="fas fa-edit" style="color: blue;"></i>
                                        </a>

                                        <!-- Delete Button -->
                                        <a href="<?= base_url('pengguna/pengobatan/delete/' . $item['id_pengobatan']) ?>" class="btn btn-danger btn-sm" title="Hapus" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
