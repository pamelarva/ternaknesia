  ...
  <div class="container mt-5">
    <div class="row">
      <div class="col-md-8 offset-md-2">
        ...
        <form method="POST" action="<?= base_url('pengguna/produksi/store'); ?>" enctype="multipart/form-data">
          <div class="form-group">
            ...
            <select class="form-control" id="jenis_ternak" name="jenis_ternak" required>
              ...
              <?php foreach ($ternaks as $ternak): ?>
                <option value="<?= $ternak['id_ternak']; ?>" <?= old('jenis_ternak') == $ternak['id_ternak'] ? 'selected' : ''; ?>><?= $ternak['jenis_ternak']; ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="form-group">
            <label for="tanggal_produksi">Tanggal Produksi</label>
            <input type="date" class="form-control" id="tanggal_produksi" name="tanggal_produksi" value="<?= old('tanggal_produksi'); ?>" required>
          </div>
          <div class="form-group">
            <label for="jenis_produksi">Jenis Produksi</label>
            <select class="form-control" id="jenis_produksi" name="jenis_produksi" required>
              <option value="Susu" <?= old('jenis_produksi') == 'Susu' ? 'selected' : ''; ?>>Susu</option>
              <option value="Telur" <?= old('jenis_produksi') == 'Telur' ? 'selected' : ''; ?>>Telur</option>
            </select>
          </div>
          <div class="form-group">
            <label for="jumlah_produksi">Jumlah Produksi</label>
            <input type="text" class="form-control" id="jumlah_produksi" name="jumlah_produksi" value="<?= old('jumlah_produksi'); ?>" required>
          </div>
          <div class="form-group">
