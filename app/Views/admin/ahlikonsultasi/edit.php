            ...
            <div class="container-fluid pt-4 px-4">
                <div class="bg-light rounded p-4">
                    <div class="d-flex justify-content-between mb-4">
                        <h6 class="mb-0">Edit Ahli Konsultasi</h6>
                    </div>
                    <div class="table-responsive">
                        <!-- Form untuk mengedit ahli konsultasi -->
                        <form action="<?= base_url('admin/konsultasi/update/' . $ahli_konsultasi['id_ahlikonsultasi']); ?>" method="POST">
                            <!-- CSRF Token -->
                            <?= csrf_field(); ?>

                            <!-- Menambahkan metode PUT untuk form update -->
                            <input type="hidden" name="_method" value="PUT">

                            <!-- Nama Ahli -->
                            <div class="mb-3">
                                <label for="nama_ahli" class="form-label">Nama Ahli</label>
                                <input type="text" class="form-control" id="nama_ahli" name="nama_ahli" value="<?= old('nama_ahli', $ahli_konsultasi['nama_ahli']); ?>" required>
                            </div>

                            <!-- Catatan -->
                            <div class="mb-3">
                                <label for="catatan" class="form-label">Catatan</label>
                                <textarea class="form-control" id="catatan" name="catatan" rows="3" required><?= old('catatan', $ahli_konsultasi['catatan']); ?></textarea>
                            </div>

                            <!-- Nomor HP -->
                            <div class="mb-3">
                                <label for="no_hp" class="form-label">Nomor HP</label>
                                <input type="tel" class="form-control" id="no_hp" name="no_hp" value="<?= old('no_hp', $ahli_konsultasi['no_hp']); ?>" required>
                            </div>

                            <!-- Email -->
                            <div class="mb-3">
                                <label for="email" class="form-label">Email</label>
                                <input type="email" class="form-control" id="email" name="email" value="<?= old('email', $ahli_konsultasi['email']); ?>" required>
                            </div>

                            <!-- Pendidikan -->
                            <div class="mb-3">
                                <label for="pendidikan" class="form-label">Pendidikan</label>
                                <input type="text" class="form-control" id="pendidikan" name="pendidikan" value="<?= old('pendidikan', $ahli_konsultasi['pendidikan']); ?>" required>
                            </div>

                            <!-- Link GitHub -->
                            <div class="mb-3">
                                <label for="link_github" class="form-label">Link GitHub</label>
                                <input type="url" class="form-control" id="link_github" name="link_github" value="<?= old('link_github', $ahli_konsultasi['link_github']); ?>">
                            </div>

                            <!-- Link Instagram -->
                            <div class="mb-3">
                                <label for="link_instagram" class="form-label">Link Instagram</label>
                                <input type="url" class="form-control" id="link_instagram" name="link_instagram" value="<?= old('link_instagram', $ahli_konsultasi['link_instagram']); ?>">
                            </div>

                            <!-- Lokasi -->
                            <div class="mb-3">
                                <label for="lokasi" class="form-label">Lokasi</label>
                                <input type="text" class="form-control" id="lokasi" name="lokasi" value="<?= old('lokasi', $ahli_konsultasi['lokasi']); ?>">
                            </div>
